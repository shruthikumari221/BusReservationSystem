...
public class BusServiceImpl implements BusService {
    ...
    public BusServiceImpl(BusDao busdao, SessionRepo srepo, RouteRepo rrepo) {
        this.busdao = busdao;
        this.srepo = srepo;
        this.rrepo = rrepo;
    }
...
//		If user not logged in throw User Exception
        if (loggedInUser == null) throw new UserException("Please provide a valid key to update Bus");
        if ("Admin".equalsIgnoreCase(loggedInUser.getType())) {
            var opt = busdao.findById(bus.getBusId());
            if (opt.isPresent()) {
                Bus curr = opt.get();
//			if seat is not available you cannot update bus details
                if (!curr.getAvailableSeats().equals(curr.getSeats()))
...
//		anyone can view bus but it must be user or admin
        ...
        if (buslist.isEmpty()) throw new BusException("bus list is empty");
        return buslist;
